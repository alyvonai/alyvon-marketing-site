// IconWrapper: enforces the 24px icon box (h-6 w-6) and a fixed 2px stroke width across
// every lucide-react icon, per the ratified icon-system spec (2026-07-29): 2px stroke,
// 24x24 viewBox, line-only icons — superseding the earlier 1.5px draft rule. Accepts an Icon
...
const IconWrapper = React.forwardRef<HTMLSpanElement, IconWrapperProps>(
  ({ className, icon: Icon, "aria-label": ariaLabel, ...props }, ref) => (
    <span
      ...
      {...props}
    >
      <Icon className="h-full w-full" strokeWidth={2} aria-hidden="true" />
    </span>
  )
)
